...
export default class FriendshipTracker {
  ...
  addFriendRequest = (fromUser, toUser, callback) => {
    ...
    firebaseFriendship.addFriendRequest(fromUser, toUser, this.persistFriendshipsCounts, this.extendFollowers, this.enableFeedUpdates, response => {
      if (this.extendFollowers == false) {
        // We added someone as a friend, so we populate both timelines if the users just became friends
        const friendships = state.friends.friendships;
        const detectedFriendship = friendships.find(friendship => friendship.user.id == toUser.id && friendship.type == FriendshipConstants.FriendshipType.inbound);
  ...
  hydrateFriendships() {
    ...
    if (hydratedUsers
      ...
      const hydratedFriends = hydratedUsers.filter(user => outboundFriendsIDs[user.id] == true && inboundFriendsIDs[user.id] == true); // reciprocal friendship (Facebook style)
      ...
      // We remove all friends and friendships from banned users
      var finalFriendships = [...friendshipsFromInbound, ...friendshipsFromFriends, ...friendshipsFromOutbound].filter(friendship => !bannedUserIDs.includes(friendship.user.id));
      if (this.extendFollowers) {
        // for follows, we need to dedup, since outbound and mutual relationships both show up as friends
