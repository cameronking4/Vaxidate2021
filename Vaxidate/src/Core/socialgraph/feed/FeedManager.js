...
export default class FeedManager {
  ...
  subscribeIfNeeded = () => {
    ...
    if (!state.feed.didSubscribeToMainFeed) {
      ...
      this.discoverUnsubscribe = firebasePost.subscribeToDiscoverFeedPosts(this.onDiscoverPostsUpdate);
  ...
  hydratePostsIfNeeded = (skipReactionReduxUpdate = false) => {
    ...
    if (this.reactions && this.discoverPosts) {
      this.reduxStore.dispatch(setDiscoverFeedPosts(this.hydratedPostsWithReactions(this.discoverPosts, this.reactions, this.bannedUserIDs)));
    }
  }
